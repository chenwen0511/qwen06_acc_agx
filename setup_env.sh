#!/bin/bash
# Qwen2.5-0.5B AGX 环境安装脚本（Orin JetPack 6.x）
set -euo pipefail
...
if [ ! -f lib/libcusparseLt.so.0 ]; then
  ...
  curl -Ls "$CUSPARSELT_URL" -o /tmp/cusparselt/archive.tar.xz
...
echo "安装完成。Phase 1 Transformers："
echo "  export LD_LIBRARY_PATH=$ROOT/lib:\$LD_LIBRARY_PATH"
echo "  source venv/bin/activate"
echo "  python infer_hf.py --device cuda:0"
echo ""
echo "Phase 2 TensorRT Edge-LLM："
echo "  bash setup_edgellm.sh"
echo "  bash acc/export_onnx.sh && bash acc/build_engine.sh"
